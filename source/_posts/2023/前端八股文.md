...
### Vue diff 的详细过程
  ...
  - vue中diff的步骤分析
    1. 在 update() 方法的内部,执行 ptach 方法`给真实的DOM打补丁`,接受两个参数,oldvnode和newvnode
...
这么设计的目的：
1. 在底层磨平不同浏览器的差异，React实现了统一的事件机制，我们不再需要处理浏览器事件机制方面的兼容问题
...
- React实现事件的过程
  1. 绑定（建立合成事件与原生事件的对应关系）
    ...
    ```
  2. 派发(针对同一个事件，即使可能存在多次回调，document（container）也只需要注册一次监听)
    - React注册到document(container)上的并不是一个某个DOM节点具体的回调逻辑，而是一个统一的事件分发函数dispatchEvent - > 事件委托思想
    - 事件触发的本质是对dispatchEvent函数的调用

- 事件工作流程
  1. 事件触发，冒泡至document(container)
  2. 执行dispatchEvent
  3. 创建事件对应的合成事件对象（SyntheticEvent）
  4. 收集事件在**捕获阶段**所波及的**回调函数**和**节点实例**
  5. 收集事件在**冒泡阶段**所波及的**回调函数**和**节点实例**
  6. 将**前两步收集来的回调**按顺序执行，执行时（SyntheticEvent）会作为参数传进去。
...
1. 类组件有生命周期钩子，函数组件没有
1. 类组件可以定义并维护**自己的state**，属于**有状态组件**，函数组件是**无状态组件**
...
- React hooks https://juejin.cn/post/7118937685653192735
  - 优点：
    ...
    1. 解决业务逻辑难以拆分的问题
    1. 使状态逻辑复用变的简单可行
    1. 函数组件从设计理念来看，更适合react
  - 局限性：
    1. hooks还不能完整的为函数组件提供类组件的能力
    1. 函数组件给了我们一定程度的自由，却也对开发者的水平提出了更高的要求
    1. Hooks 在使用层面有着严格的规则约束
